...
# Aggregate by daily temp max
df_2020_daily <- df_2020 |> 
  group_by(time) |> 
  summarise(temperature = max(temperature_2m_C, na.rm = TRUE))
...
humdew_max_2024 <- df_2024 |> 
  group_by(time) |> 
  summarise(humidity = max(relative_humidity_2m, na.rm = TRUE),
            dew_point = max(dew_point_2m_C, na.rm = TRUE))
...
ggplot(df_daily, aes(x = time, y = temperature, color = year)) +
  geom_line() +
  labs(title = "Daily Max. Temperature Trends (2020 vs. 2024)", 
...
hchart(df_2020_daily, "line", hcaes(x=time, y=temperature), name = "2020 Daily Max. Temperature", showInLegend = TRUE) |> 
