async function drawLineChart2024 () {

    // 1. Access the data
    // 1.1 Load the data

    const kiziridataset2024 = await d3.csv("../R/data/kiziri/kiziri_daily_temp_2024.csv", d3.autoType) //d3.autoType method automatically changes the max-temp & date to JS format
    ...
    // console.log(yAccessor(kiziridataset2024[0])); // confirm by console loging

    // console.log(xAccessor(kiziridataset2024[0])); // confirm by console loging
    ...
    const wrapper = d3.select("#kiziri-chart-2") 
      .append("svg")
    ...
    const yScale = d3.scaleLinear()
      .domain(d3.extent(kiziridataset2024, yAccessor)) // Outputs the min & max values of our dataset2
     ...
     // console.log(d3.extent(kiziridataset2024, yAccessor)); // confirm by console logging it
    

    // 4.3 Creating the xScale with d3.scaleTime() method since we are dealing with dates

    const xScale = d3.scaleTime()
      .domain(d3.extent(kiziridataset2024, xAccessor)) // Outputs the min & max values of our dataset2
      .range([0, dimensions.boundedWidth])  // Outputs the max Width value "boundedWidth" of our screen dimension within the bounded area

     // console.log(d3.extent(kiziridataset2024, xAccessor)); // confirm by console logging it
    ...
    const line = bounds.append("path")
      .attr("d", lineGenerator(kiziridataset2024))
      .attr("fill", "none")
      .attr("stroke", "#af9358")
      .attr("stroke-width", 2)
